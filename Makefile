...
typed-rt: min-caml
	./min-caml -tprint min-rt/min-rt;
	#ocamlformat min-rt/min-rt.t.ml > min-rt/min-rt.f.ml;
	#mv min-rt/min-rt.f.ml min-rt/min-rt.t.ml;

itc-rt: typed-rt
	../incremental-mincaml/main.native min-rt/min-rt.t.ml min-rt/min-rt.t.ml

run-ttests: typed-rt
	rm test/*.t.ml;
	for i in test/*.ml; do echo $i; ./min-caml -tprint "test/`basename $i .ml`"; done;
	echo "--------------------";
	for i in test/*.t.ml; do ../incremental-mincaml/main.native $i $i; done
